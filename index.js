...
function populateTrendingSearch1(res) {
	...
	res.forEach((data, index) => {
		html += `<div id="searchresbox1" class="commonFlex contentBetween itemscenter spacer positionRelative">
...
function populateTrendingSearch2(res) {
	...
	res.forEach((data, index) => {
		html += `<div id="searchresbox2" class="commonFlex contentBetween itemscenter spacer positionRelative">
			<a href="${CURRLANG}currencies/${data.slug}/" class="positonedLink"></a>
			<div class="left commonFlex itemscenter">
...
function buildSearchList(resid, skey, list) {
	var outputbox = document.getElementById(resid);
	var output = null;
	...
	if (searchstandard == '') {
		searchstandard = outputbox.innerHTML;
	}

	if (skey == '') {
		outputbox.innerHTML = searchstandard;
		return false;
	}
	...
	list.forEach((data, index) => {
		html += `<div id="outputbox" class="commonFlex contentBetween itemscenter spacer positionRelative">
					<a  href="${CURRLANG}currencies/${data.slug}/" class="positonedLink"></a>
					<div class="left commonFlex itemscenter">
...
document.addEventListener('DOMContentLoaded', () => {
	...
	document.querySelector('.custmonDropDwon').addEventListener('click', function (e) {
			if (e.target.contains('crossString') || e.target.contains('bowiro')) {
				return false;
			}
			...
			if (
				e.target.contains('language') ||
				e.target.parentNode.contains('language')
			) {
				languageFunc();
			}
			...
			const nextCustomDrp = this.nextElementSibling || this.children[0];
			if (nextCustomDrp) {
	...
	const bannerreadbtn = document.querySelector('.basic_banner_read_btn');
	bannerreadbtn.addEventListener('click', () => {
		if ((basic_banner_read_btn = false)) {
			bannerreadbtn.textContent = 'Read Less';
			basic_banner_read_btn = true;
			if (bannerreadbtn.clientHeight == 0) {
				bannerreadbtn.style.height = '500';
			} else {
				bannerreadbtn.style.height = '0';
			}
		} else {
			bannerreadbtn.textContent = 'Read More';
			if (bannerreadbtn.clientHeight == 500) {
				bannerreadbtn.style.height = '0';
			} else {
				bannerreadbtn.style.height = '500';
			}
	...
	const closeIconFn = () => {
		const closeIconSel = document.querySelector('.closeIcon');
		closeIconSel.addEventListener('click', () => {
			const customPop = document.querySelector('.customPopup');
			customPop.classList.remove('active');
			const commonPopup = document.querySelector('.commonPopup');
			commonPopup.classList.remove('active');
		});
	};
	closeIconFn();

	const responsiveTables = (selector, child) => {
		selector.addEventListener('scroll', e => {
			var isScroll = e.currentTarget.scrollLeft;
			if (isScroll) {
				document
					.querySelector(
						`.ListingTable th:nth-child(${child}), td:nth-child(${child})`
					)
	...
	responsiveTables('.mainTable', 3);
	// ==  responsiveTable currenciesTable == //
	responsiveTables('.currenciesTable, .exchangeTable', 2);
	// == reponsiveTable historical-data
	responsiveTables('.historical-data', 1);
	// == reponsiveTable financialTable
	responsiveTables('.financialTable', 1);
	...
	var isHighLight = document.querySelector('#HighlightsBox');
...
// == == == faq scripts == == == //
const faqHead = document.querySelector('.faqAcc .flexAccHead');
faqHead.addEventListener('click', event => {
	let AccHeadObj = event.target;
	AccHeadObj.querySelector('>').classList.remove('active');
	AccHeadObj.querySelector('.faqDesc').classList.add('active');
});
// == === == faq scripts == == == //
...
function chart_switchdata(id, elem, event) {
...
function chart_switchview(id, elem, event) {
...
function toggleCurrencyMoreData(elem) {
	...
	var currentstate = elem.innerText;
	var alternate = elem.dataset.alternate;
	var currencyBottomDeats = document.querySelector(
		'.currencyDetail .bottomListin'
	);
	if (currentstate == alternate) {
		currencyBottomDeats.style.display = 'none';
		elem.innerHTML = toggleCurrencyMoreDataState;
	} else {
		currencyBottomDeats.style.display = 'block';
		elem.innerHTML = alternate;
	}
}
